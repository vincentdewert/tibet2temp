...
public class ManageEmsPipe extends TimeoutGuardPipe {
	...
	public String doPipeWithTimeoutGuarded(Object input,
		...
		try {
			...
			if (action_work == null) {
				return getXmlErrorMessage("unknown action [" + action_temp
						+ "]");
			}

			if (StringUtils.isEmpty(queueName_work)) {
				return getXmlErrorMessage("queueName must be filled");
			}

			if (!queueName_work.startsWith("ABC.")) {
				return getXmlErrorMessage("queueName must start with 'ABC.'");
			}
			...
			if (action_work.equals(CREATE_QUEUE)) {
				if (queueInfo == null) {
					...
					return getXmlErrorMessage("queue [" + queueInfo.getName()
							+ "] already exists");
				}
			} else {
				if (queueInfo == null) {
					String message = "queue [" + queueName_work
							+ "] does not exist";
					if (action_work.equals(DELETE_QUEUE)) {
						return getXmlWarnMessage(message);
					} else {
						return getXmlErrorMessage(message);
					}
				}
				if (action_work.endsWith("QueueAuthorization")) {
					if (StringUtils.isEmpty(principal_work)) {
						if (!action_work.equals(READ_QUEUE_AUTHORIZATION)) {
							return getXmlErrorMessage("principal must be filled");
						}
					} else {
						userInfo = admin.getUser(principal_work);
						if (userInfo == null) {
							return getXmlErrorMessage("principal ["
									+ principal_work + "] does not exist");
						}
					}
				}
			}
			...
			if (action_work.equals(CREATE_QUEUE)) {
				...
				result = createQueueAuthorization(admin, queueInfo, userInfo,
						permissions_work);
			} else if (action_work.equals(READ_QUEUE_AUTHORIZATION)) {
				result = readQueueAuthorization(admin, queueInfo, userInfo,
						permissions_work);
			} else if (action_work.equals(UPDATE_QUEUE_AUTHORIZATION)) {
				result = updateQueueAuthorization(admin, queueInfo, userInfo,
						permissions_work);
			} else if (action_work.equals(DELETE_QUEUE_AUTHORIZATION)) {
				result = deleteQueueAuthorization(admin, queueInfo, userInfo,
						permissions_work);
			}
		} catch (Exception e) {
			String msg = getLogPrefix(session)
					+ "exception on managing Tibco EMS, action [" + action_work
					+ "] url [" + url_work + "]";
	...
	private String createQueue(TibjmsAdmin admin, QueueInfo queueInfo)
			throws PipeRunException, TibjmsAdminInvalidNameException,
			TibjmsAdminException {
		admin.createQueue(queueInfo);
		// TODO: update some attributes initially (prefetch) and attributes
		// from body
		return getXmlOkayMessage("queue [" + queueInfo.getName()
				+ "] is created");
	}

	private String readQueue(TibjmsAdmin admin, QueueInfo queueInfo)
			throws PipeRunException, TibjmsAdminInvalidNameException,
			TibjmsAdminException {
		return getXmlOkayMessage(queueInfo.toString());
	}

	private String updateQueue(TibjmsAdmin admin, QueueInfo queueInfo)
			throws PipeRunException, TibjmsAdminInvalidNameException,
			TibjmsAdminException {
	...
	private String deleteQueue(TibjmsAdmin admin, QueueInfo queueInfo)
			throws PipeRunException, TibjmsAdminInvalidNameException,
			TibjmsAdminException {
		...
		if (aclEntries == null || aclEntries.length == 0) {
			...
			return getXmlErrorMessage("queue [" + queueInfo.getName()
					+ "] must not no authorizations");
		}
	}

	private String createQueueAuthorization(TibjmsAdmin admin,
			QueueInfo queueInfo, UserInfo userInfo, String permissions)
			throws PipeRunException, TibjmsAdminInvalidNameException,
			TibjmsAdminException {
		ACLEntry[] aclEntries = admin.getQueueACLEntries(queueInfo.getName());
		ACLEntry aclEntry = retrieveACLEntry(aclEntries, userInfo);
		if (aclEntry != null) {
			return getXmlErrorMessage("authorization for principal ["
					+ userInfo.getName() + "] on queue ["
					+ queueInfo.getName() + "] already exists");
		} else {
			if (StringUtils.isEmpty(permissions)) {
				return getXmlErrorMessage("permissions must be filled");
			} else {
				grantPermissions(admin, queueInfo, userInfo, permissions);
				return getXmlOkayMessage("authorization for principal ["
						+ userInfo.getName() + "] on queue ["
						+ queueInfo.getName() + "] is created");
			}
		}
	}

	private void grantPermissions(TibjmsAdmin admin, QueueInfo queueInfo,
			UserInfo userInfo, String permissions) throws PipeRunException,
			TibjmsAdminInvalidNameException, TibjmsAdminException {
	...
	private String readQueueAuthorization(TibjmsAdmin admin,
			QueueInfo queueInfo, UserInfo userInfo, String permissions)
			throws PipeRunException, TibjmsAdminInvalidNameException,
			TibjmsAdminException {
		ACLEntry[] aclEntries = admin.getQueueACLEntries(queueInfo.getName());
		if (userInfo == null) {
			if (aclEntries == null || aclEntries.length == 0) {
				return getXmlOkayMessage("no authorization for any principal on queue ["
						+ queueInfo.getName() + "] does not exists");
			} else {
				String message = "";
	...
	private String updateQueueAuthorization(TibjmsAdmin admin,
			QueueInfo queueInfo, UserInfo userInfo, String permissions)
			throws PipeRunException, TibjmsAdminInvalidNameException,
			TibjmsAdminException {
		ACLEntry[] aclEntries = admin.getQueueACLEntries(queueInfo.getName());
		...
		grantPermissions(admin, queueInfo, userInfo, permissions);
		return getXmlOkayMessage("authorization for principal ["
				+ userInfo.getName() + "] on queue [" + queueInfo.getName()
				+ "] is updated");
	}

	private String deleteQueueAuthorization(TibjmsAdmin admin,
			QueueInfo queueInfo, UserInfo userInfo, String permissions)
			throws PipeRunException, TibjmsAdminInvalidNameException,
			TibjmsAdminException {
		ACLEntry[] aclEntries = admin.getQueueACLEntries(queueInfo.getName());
		ACLEntry aclEntry = retrieveACLEntry(aclEntries, userInfo);
		if (aclEntry == null) {
			return getXmlWarnMessage("authorization for principal ["
					+ userInfo.getName() + "] on queue [" + queueInfo.getName()
					+ "] does not exists");
		} else {
			admin.revoke(aclEntry);
	...
	private String getXmlOkayMessage(String message) {
		return "<ok>" + message + "</ok>";
	}

	private String getXmlWarnMessage(String message) {
		return "<warn>" + message + "</warn>";
	}

	private String getXmlErrorMessage(String message) {
