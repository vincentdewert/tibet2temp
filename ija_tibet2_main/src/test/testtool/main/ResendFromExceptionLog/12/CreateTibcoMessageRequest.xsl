			...
			<xsl:otherwise>
				...
				<Request>
					<QueueConnectionFactory>
						<xsl:choose>
							<xsl:when test="contains(upper-case($esb_rr_url), upper-case($provUrl))">
								<xsl:text>qcf_tibco_esb_rr</xsl:text>
							</xsl:when>
							<xsl:when test="contains(upper-case($esb_ff_url), upper-case($provUrl))">
								<xsl:text>qcf_tibco_esb_ff</xsl:text>
							</xsl:when>
							<xsl:when test="contains(upper-case($esb_rr_large_url), upper-case($provUrl))">
								<xsl:text>qcf_tibco_esb_rr_large</xsl:text>
							</xsl:when>
							<xsl:when test="contains(upper-case($p2p_rr_url), upper-case($provUrl))">
								<xsl:text>qcf_tibco_p2p_rr</xsl:text>
							</xsl:when>
							<xsl:when test="contains(upper-case($p2p_ff_url), upper-case($provUrl))">
								<xsl:text>qcf_tibco_p2p_ff</xsl:text>
							</xsl:when>
							<xsl:when test="contains(upper-case($p2p_rr_large_url), upper-case($provUrl))">
